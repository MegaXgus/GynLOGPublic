...
public class TelaDeMovimentacao extends javax.swing.JFrame {
  ...
  private MovimentacaoDAO movDao = new MovimentacaoDAO();
    ...
    /**
     * Creates new form TelaDeMovimentacao
     */
...
private void bloquearCampos() {
    ...
    jComboBoxStatus.setEnabled(false); // garante que fique desabilitado
}

// substituir liberarCampos()
private void liberarCampos() {
    // ID da movimentação fica editável apenas no modo CADASTRAR (para informar novo ID)
    jTextFieldMovimentacao.setEditable(true);
    jTextFieldveiculos.setEditable(true);
    jTextFieldDespesa.setEditable(true);
    jTextFieldData.setEditable(true);
    jTextFieldValor.setEditable(true);
    jTextFieldDescricao.setEditable(true);
    jComboBoxStatus.setEnabled(true); // habilita só quando liberar explicitamente
}

// substituir limparCampos()
...
private void carregarTabela() {
    try {
        ...
        for (Movimentacao m : lista) {
            ...
            // pegar status diretamente do objeto Movimentacao
            String status = "";
            try {
                Object s = null;
                try {
  ...
  try {
        if ("CADASTRAR".equals(modo)) {
            ...
            String idTipoStr = jTextFieldDespesa.getText().trim();
            String data = jTextFieldData.getText().trim();
            String valorStr = jTextFieldValor.getText().trim();
            String descricao = jTextFieldDescricao.getText().trim();
            String statusSelecionado = String.valueOf(jComboBoxStatus.getSelectedItem());

            // Validações
            if (idMovStr.isEmpty() || idVeicStr.isEmpty() || idTipoStr.isEmpty()
                    || data.isEmpty() || valorStr.isEmpty() || descricao.isEmpty()) {
            ...
            if (movDao.buscarPorId(idMov) != null) {
                JOptionPane.showMessageDialog(this, "Já existe movimentação com ID " + idMov);
                return;
            }

            Veiculo v = veicDao.buscarPorId(idVeic);
            if (v == null) {
                JOptionPane.showMessageDialog(this, "Veículo não encontrado (ID " + idVeic + ").");
                return;
            }
            TipoDeDespesa t = tipoDao.buscarPorId(idTipo);
            if (t == null) {
                JOptionPane.showMessageDialog(this, "Tipo de despesa não encontrado (ID " + idTipo + ").");
                return;
            }

            // Criar objeto usando o construtor com status (ou usar o construtor sem status e depois setStatus)
            Movimentacao nova = new Movimentacao(idMov, idVeic, idTipo, descricao, data, valor, statusSelecionado);
        ...
        if ("EDITAR".equals(modo)) {
            ...
            if (existente == null) {
                JOptionPane.showMessageDialog(this, "Movimentação não encontrada (ID " + idMov + ").");
                return;
            }

            // Validar e atribuir campos
            int idVeiculo = Integer.parseInt(jTextFieldveiculos.getText().trim());
            int idTipo = Integer.parseInt(jTextFieldDespesa.getText().trim());
            String data = jTextFieldData.getText().trim();
            double valor = Double.parseDouble(jTextFieldValor.getText().trim());
            String descricao = jTextFieldDescricao.getText().trim();
            String statusSelecionado = String.valueOf(jComboBoxStatus.getSelectedItem());

            existente.setIdVeiculo(idVeiculo);
            existente.setIdTipoDespesa(idTipo);
            existente.setData(data);
            existente.setValor(valor);
            existente.setDescricao(descricao);
            existente.setStatus(statusSelecionado);
        ...
        // Se não estiver em nenhum modo
        JOptionPane.showMessageDialog(this, "Nenhuma operação selecionada. Clique em 'Cadastrar Movimentação' ou 'Gerenciar Movimentação' antes de confirmar.");
    } catch (NumberFormatException nfe) {
        JOptionPane.showMessageDialog(this, "Verifique os valores numéricos (IDs ou valor).");
    } catch (Exception e) {
        JOptionPane.showMessageDialog(this, "Erro ao confirmar movimentação: " + e.getMessage());
    ...
    private void jButtonCadastrarMovimentacaoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonCadastrarMovimentacaoActionPerformed

        // preparar formulário para cadastrar (não salva aqui)
        modo = "CADASTRAR";
        limparCampos();
        liberarCampos();
        // no cadastro, ID da movimentação deve ser informado
        jTextFieldMovimentacao.setEditable(true);
 ...
 try {
            ...
            jTextFieldDespesa.setText(String.valueOf(m.getIdTipoDespesa()));
            ...
            modo = "EDITAR";
            // ID da movimentação não deve ser editável ao editar
            jTextFieldMovimentacao.setEditable(false);
            jTextFieldveiculos.setEditable(true);
            jTextFieldDespesa.setEditable(true);
            jTextFieldData.setEditable(true);
